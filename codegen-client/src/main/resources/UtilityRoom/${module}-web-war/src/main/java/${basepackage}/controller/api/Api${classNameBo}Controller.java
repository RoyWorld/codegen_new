...
import ${basepackage}.service.impl.I${className}ServiceImpl;
import ${basepackage}.domain.SearchField;
import ${basepackage}.domain.BizData4Page;
import ${basepackage}.domain.RtnCodeEnum;
...
public class Api${className}Controller{
    ...
    @Autowired
    private I${className}ServiceImpl ${classNameLower}ServiceImpl;
...
<#list table.columns as column>
    <#if column != 'id' && !column.isNullable()>
        ...
        <#else>
            ...
            int row = ${classNameLower}ServiceImpl.insert(${classNameLower});
            if(row > 0) {
                return new String( StringUtil.toString(${classNameLower}.getId()) );
            }else{
    ...
    public Object del${className}(@PathVariable String ${classNameLower}Id) {
        try {
            ...
            condition.put("id", ${classNameLower}Id);
            int row = ${classNameLower}ServiceImpl.deleteByCondition(condition);
            if(row > 0) {
                return new String( "删除${table.name}成功" );
            }else{
    ...
    public String edit${className}(@RequestBody ${className} ${classNameLower}, @PathVariable String ${classNameLower}Id) {
        try {
            String msg = "";
            if(${classNameLower}==null) {
                msg = "添加${table.name}参数不正确";
<#list table.columns as column>
    <#if column != 'id' && !column.isNullable()>
        ...
        <#if column.possibleShortJavaType != 'java.util.Date'>
            ...
            Map<String, Object> condition = new HashMap<>();
            condition.put("id", ${classNameLower}Id);
            ${className} ${classNameLower}_old = (${className}) ${classNameLower}ServiceImpl.queryOne(condition);
            if(${classNameLower}==null) {
...
<#list table.columns as column>
    ...
    <#else>
            ...
            int row = ${classNameLower}ServiceImpl.update(${classNameLower}_old);
            if(row > 0) {
                return new String("修改${table.name}成功");
            }else{
    ...
    public ${className} get${className}(@PathVariable String ${classNameLower}Id) {
        try {
            ...
            ${className} ${classNameLower}= (${className}) ${classNameLower}ServiceImpl.queryOne(whereParams);
...
<#list table.columns as column>
    ...
    <#else>
            ...
            ${classNameLower}ServiceImpl.queryPageByDataPerm(bizData4Page);
            return bizData4Page;
        } catch (BizException bize) {
            throw bize;
        } catch (Exception e) {
